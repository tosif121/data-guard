  ...
  const handleExecute = async () => {
    ...
    if (actionKey === "rollback") {
      try {
        ...
        const { initiateRollback } = await import("@/lib/supabase");
        const result = await initiateRollback("payment-service"); // Hardcoded for demo/simplicity
        if (!result.success) {
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col items-end gap-2">
          <span
            className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${badgeColor}`}
          >
            {meta.risk} Risk
          </span>
